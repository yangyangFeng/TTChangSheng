...
//遮罩浪
@property (nonatomic, strong) CAShapeLayer *maskWave2Layer;
...
- (void)initData{
    ...
    self.maskWave2Color = [[UIColor whiteColor] colorWithAlphaComponent:0.4];
    [self.layer addSublayer:self.realWaveLayer];
    [self.layer addSublayer:self.maskWaveLayer];
    [self.layer addSublayer:self.maskWave2Layer];
}
...
- (CAShapeLayer *)maskWave2Layer{
    
    if (!_maskWave2Layer) {
        _maskWave2Layer = [CAShapeLayer layer];
        CGRect frame = [self bounds];
        frame.origin.y = frame.size.height-self.waveHeight;
        frame.size.height = self.waveHeight;
        _maskWave2Layer.frame = frame;
        _maskWave2Layer.fillColor = self.maskWave2Color.CGColor;
    }
    return _maskWave2Layer;
}
...
- (void)wave{
    ...
    for (CGFloat x = 0.f; x <= width ; x++) {
        ...
        path3Y = height * sinf([self getSinfWithPercent:1.6f widthX:x]);
    ...
    self.maskWave2Layer.path = path3;
    self.maskWave2Layer.fillColor = self.maskWave2Color.CGColor;
    CGPathRelease(path3);
}
