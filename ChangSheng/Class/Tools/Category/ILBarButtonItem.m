...
+ (ILBarButtonItem *)rightBarItemWithImage:(UIImage *)image
                             selectedImage:(UIImage *)selectedImage
                                    target:(id)target
                                    action:(SEL)action
{
    return [[ILBarButtonItem alloc] initRightWithImage:image
                                    selectedImage:selectedImage
                                           target:target
                                           action:action];
}

- (id)initRightWithImage:(UIImage *)image
      selectedImage:(UIImage *)selectedImage
             target:(id)target action:(SEL)action
{
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    [btn setFrame:CGRectMake(.0f, 0.0f, 18.0f, 18.0f)];
...
- (id)initWithImage:(UIImage *)image
      selectedImage:(UIImage *)selectedImage
             target:(id)target action:(SEL)action
{    
    IBCustomButton *btn = [IBCustomButton buttonWithType:UIButtonTypeCustom];
    [btn setFrame:CGRectMake(.0f, 0.0f, 30.0f, 30.0f)];
    [btn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    [btn setImage:image forState:UIControlStateNormal];
    [btn setImage:selectedImage forState:UIControlStateHighlighted];
