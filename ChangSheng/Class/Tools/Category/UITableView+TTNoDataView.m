...
#import "UITableView+TTNoDataView.h"
...
@implementation UITableView (TTNoDataView)
...
//        [self addSubview:self.tt_contentView];
        self.tableFooterView = self.tt_contentView;
...
- (void)tt_checkNoDataViewTitleInFooter:(NSString *)title withDataCount:(NSInteger )count height:(CGFloat)height
{
    if (!count) {
        ...
        self.tt_contentView = [[UIView alloc]initWithFrame:self.bounds];
        self.tt_contentView.height = height;
        self.tt_contentView.backgroundColor = self.backgroundColor;
        
        
        self.tt_noDataView = [[UIImageView alloc]initWithImage:IMAGE(@"nodataicon")];
        self.tt_noDataView.frame = CGRectMake(self.tt_contentView.size.width /2.0 - 100/2.0, self.tt_contentView.frame.size.height/7.0 , 100, 100);
