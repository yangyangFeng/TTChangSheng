...
//    [Bugly startWithAppId:@"0c0be2e975" config:config];
    ...
    [JPUSHService setupWithOption:launchOptions appKey:@"bbf1c37202df53c1a141b34a"
                          channel:nil
                 apsForProduction:NO
            advertisingIdentifier:nil];
    
    [JPUSHService registrationIDCompletionHandler:^(int resCode, NSString *registrationID) {
        if (resCode == 0 && [CSUserInfo shareInstance].isOnline){
            
            CSDeviceBindingModel * param = [CSDeviceBindingModel new];
            param.device_id = registrationID;
            ...
            DLog(@"registerID 获取成功:%@",registrationID);
        }
    }];
}
